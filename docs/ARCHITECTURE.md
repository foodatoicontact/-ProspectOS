...
## Déploiement
Aucune base existante n’est modifiée sans identification de la cible. Variables : NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY, AI_PROVIDER, AI_API_KEY, AI_MODEL, SUPABASE_SERVICE_ROLE_KEY (serveur uniquement — jamais préfixée NEXT_PUBLIC_, jamais envoyée au navigateur ; utilisée par `src/server/account.ts` pour anonymiser un compte définitivement fermé, et par `src/server/admin-client.ts` pour écrire le ledger de coût et déchiffrer une clé BYOK), BRAVE_SEARCH_API_KEY, DISCOVERY_ALLOWED_HOSTS, BYOK_MASTER_KEY (serveur uniquement — clé maîtresse AES-256-GCM du chiffrement des clés BYOK, voir docs/COST_METERING.md). Démo séparée sans compte ; production refuse les accès non authentifiés. Les secrets ne figurent jamais dans le ZIP.
