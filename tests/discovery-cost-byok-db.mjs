...
// ============================================================
// U — no client role can ever write api_usage_events directly
// ============================================================
await rejects(as(A, `insert into public.api_usage_events(organization_id,user_id,provider,operation) values ($1,$2,'brave','search')`, [OA, A]),
  /permission denied/);
...
// ============================================================
await asAdmin(`insert into prospectos_private.provider_pricing(provider,operation,model,unit_type,price_per_unit_micros,version) values ('brave','search',null,'request',5000,'v1')`);
const costV1 = (await asAdmin(`select public.resolve_provider_cost('brave','search',null,'[{"unit_type":"request","quantity":1}]'::jsonb) r`)).rows[0].r;
assert.equal(costV1.estimated_cost_micros, 5000, 'Q — cost computed from the active v1 price');
assert.equal(costV1.pricing_version, 'v1');
const eventV1 = (await asAdmin(
  `insert into public.api_usage_events(organization_id,user_id,provider,operation,estimated_cost_micros,pricing_version) values ($1,$2,'brave','search',$3,$4) returning id`,
...
await asAdmin(`update prospectos_private.provider_pricing set effective_to=now() where version='v1'`);
await asAdmin(`insert into prospectos_private.provider_pricing(provider,operation,model,unit_type,price_per_unit_micros,version) values ('brave','search',null,'request',9000,'v2')`);
const costV2 = (await asAdmin(`select public.resolve_provider_cost('brave','search',null,'[{"unit_type":"request","quantity":1}]'::jsonb) r`)).rows[0].r;
assert.equal(costV2.estimated_cost_micros, 9000, 'Q — a new call now prices at v2');
...
assert.equal(frozenEvent.estimated_cost_micros, 5000, "Q — the historical event keeps its original, frozen cost after a tariff change");
assert.equal(frozenEvent.pricing_version, 'v1');
...
// No pricing configured at all for an operation/unit_type → null, never a guess.
const noPricing = (await asAdmin(`select public.resolve_provider_cost('anthropic','offer_analysis',null,'[{"unit_type":"input_tokens_1k","quantity":2}]'::jsonb) r`)).rows[0].r;
assert.equal(noPricing, null, 'no fabricated cost when no active pricing row exists for that provider/operation/unit');

console.log('PASS discovery-cost-byok-db: cost ledger — no-client-write, tenant isolation, integer cost, frozen pricing history, append-only');
